...
import { HeadCellClients } from '@/types/clients'

const headCells: readonly HeadCellClients[] = [
  { id: 'fullName', label: 'Nome' },
  { id: 'cpf', label: 'CPF' },
  { id: 'countBudget', label: 'Orçamentos' },
  { id: 'countOrder', label: 'Pedidos' },
...
export default function ClientsPage() {
  ...
  const { clients } = useSelector(ClientState)
  const totalPages = clients.paging.totalPages
        ...
        <BodyPage>
          <TableContainer>
            ...
            >
              <EnhancedTableHead />
              <TableBody>
                {clients.result?.map((row: any, index: any) => {
                  return (
                    <TableRow
                      key={index}
                    ...
                    >
                      ...
                      <BodyTableCell>{row.countBudget}</BodyTableCell>
                      <BodyTableCell>{row.countOrder}</BodyTableCell>
                      <BodyTableCell>
                        <CustomSwitch
                          sx={{ m: 1 }}
                          checked={row.active}
