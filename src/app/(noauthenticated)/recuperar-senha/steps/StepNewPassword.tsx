...
const StepNewPassword = () => {
  ...
  const handleResetPasswordWithTokenValidation = (values: FormValues) => {
    ...
    dispatch(resetPasswordWithTokenValidation(bodyData)).then(
      (response: any) => {
      ...
      <BoxForm>
        ...
        <CustomDialog
          ...
          content={
            <>
              <Typography>
                Senha alterada com sucesso! Para acessar o sistema basta inserir
                sua nova senha na tela de login. Você será redirecionado
                automaticamente, mas caso isso não aconteça{' '}
                <Link href='/autenticacao/login'>clique aqui</Link>.
              </Typography>
            </>
          }
          fullWidthValeu={true}
