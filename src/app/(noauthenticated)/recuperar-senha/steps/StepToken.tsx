...
} from '@/store/modules/auth/authActions'
...
const StepToken = ({
  ...
  const handleFormVerifyTokenValidation = (values: FormValues) => {
    ...
    dispatch(verifyTokenValidation(bodyData)).then((response: any) => {
      if (response && response.success) {
        ...
        if (erroRegister) {
          setDialogContent(
            erroRegister.message || 'Não foi possivel validar o token.'
          )
          setOpenDialog(true)
          setLoading(false)
  ...
  const resendTokenValidation = () => {
    ...
    const emailOrMobilePhone =
      authState?.recoverPassword?.bodyAPI?.emailOrMobilePhone || ''
    const valueTokenType =
      authState?.recoverPassword?.bodyAPI?.verificationCodeType || ''
    dispatch(
      sendTokenValidation({
  ...
  return (
    <>
      <BoxForm>
        ...
        {counterClink == 2 && (
          <Stack sx={{ width: '80%', marginTop: 2 }} spacing={2}>
            <Alert severity='info'>
              Com dificuldade de realizar a redefinição de senha? Entre em
              contato conosco através do
              <strong> (99) 99999-9999</strong> para te ajudarmos a recuperar
              sua senha
            </Alert>
          </Stack>
        )}
            ...
            <Form>
              ...
              <BoxContador>
                <Typography textAlign={'center'} maxWidth={'300px'}>
                  Não recebeu? Você pode solicitar um novo token em{' '}
