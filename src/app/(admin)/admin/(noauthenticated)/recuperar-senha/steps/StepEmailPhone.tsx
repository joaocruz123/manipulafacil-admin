...
import { Box, FormControl, FormControlLabel, Grid, Radio, RadioGroup, Typography, Button } from '@mui/material'
...
const StepEmailPhone = ({ handleNextStep, handleStartCounter }: Props) => {
  ...
  const handleFormValidation = (values: FormValues) => {
    setLoading(true)
    const selectedFlow = pathname === 'admin/recuperar-senha' ? 1 : 2
    const emailOrMobilePhone = valueTokenType === 'email' ? values.email : values.phone
    dispatch(sendTokenValidation({
      ...
      .then((response: any) => {
        if (response && response.success) {
          ...
          if (erroEmail) {
            setDialogContent(erroEmail.message || 'Registro não encontrado na base.')
            setOpen(true)
            setLoading(false)
                  ...
                  <Grid container direction={'column'}>
                    <Grid item md={12}>
                      <FormControl>
                        ...
                        >
                          <FormControlLabel value='email' control={<Radio />} label='Enviar token por e-mail' />
                        </RadioGroup>
                      </FormControl>
                    </Grid>
                    ...
                    <Grid item md={12}>
                      <FormControl>
                        <RadioGroup
                          ...
                          onChange={handleChangeTokenType}
                        >
                          <FormControlLabel value='sms' control={<Radio />} label='Enviar token por SMS' />
                        </RadioGroup>
                      </FormControl>
                    </Grid>
                    {valueTokenType === 'sms' &&
              ...
              <Wrapper container >
                <Grid item xs={12} md={6}>
                  ...
                  <Grid item xs={12}>
                    <ButtonFormBack
                      color='secondary'
                      onClick={() => route.push('/farmacias/autenticacao/login')}
                      disableElevation
                    >
                      Voltar
                    </ButtonFormBack>
